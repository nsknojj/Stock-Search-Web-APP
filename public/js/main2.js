...
function showValid() {
  $('#symbol').css('border-color', 'rgb(204, 204, 204)').css('-webkit-box-shadow', 'inset 0 1px 1px rgba(0,0,0,.075), 0 0 8px rgba(102,175,233,.6)').css('box-shadow', 'inset 0 1px 1px rgba(0,0,0,.075), 0 0 8px rgba(102,175,233,.6)');
...
function showInvalid() {
  ...
  // [0].setCustomValidity('Please enter a stock ticker symbol.');
...
function submitDetail(symbol) {
  ...
  xhr['detail'] = $.ajax({
    ...
    success: function(response) {
      ...
      $("#detail-open").html(Number(open).toFixed());
...
angular
  ...
  .controller('SearchCtrl', ['$timeout', '$q', '$log', SearchCtrl])
  .controller('FavCtrl', ['$scope', '$sce', FavCtrl]);
